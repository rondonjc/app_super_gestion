    ...
    <div class="informacao-pagina" style="text-align:center; display: flex; justify-content: center">
        ...
        <div style="width: 30%; margin-left:auto; margin-right:auto;">
            ...
            <form method="POST" action="{{route('producto.store')}}">
                @csrf
                <input type="text" name="nombre" value="{{old('nombre')}}" placeholder="Nombre" class="borda-preta" >
                {{$errors->has('nombre')?$errors->first('nombre'):""}}
                <textarea name="descripcion" class="borda-preta" placeholder="Descripcion">{{(old('descripcion')!="")?old('descripcion'):''}}</textarea>
                ...
                <select name="unidad_id" class="borda-preta">
                    ...
                    @foreach ($unidades as $unidad)
                        <option {{(old('unidad_id')==$unidad->id)?'selected':''}} value="{{$unidad->id}}">{{$unidad->descripcion}}</option>
                    @endforeach
                </select>
                ...
                <select name="proveedor_id" class="borda-preta">
                    ...
                    @foreach ($proveedores as $proveedor)
                        <option {{(old('proveedor_id')==$proveedor->id)?'selected':''}} value="{{$proveedor->id}}">{{$proveedor->nombre}}</option>
                    @endforeach
                </select>
